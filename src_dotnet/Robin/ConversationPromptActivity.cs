...
/// フルスクリーンでプロンプトを編集できる
/// </summary>
[Activity(Label = "会話用プロンプト", Theme = "@style/AppTheme")]
public class ConversationPromptActivity : AppCompatActivity
{
    ...
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        // レイアウトを動的に作成（dialog_system_prompts を使わない）
        CreateLayout();
    ...
    private void CreateLayout()
    {
        ...
        _promptInput.Hint = "会話用のシステムプロンプトを入力してください";
    ...
    private void LoadSettings()
    {
        ...
        if (_promptInput != null)
        {
            _promptInput.Text = settings.ConversationPrompt ?? SystemPrompts.ConversationSystemPrompt;
    ...
    private void ResetToDefault()
    {
        if (_promptInput != null)
        {
            _promptInput.Text = SystemPrompts.ConversationSystemPrompt;
            Log.Info("ConversationPromptActivity", "デフォルトプロンプトにリセット");
    ...
    private void SaveSettings()
    {
        ...
        try
        {
            ...
            var newSettings = new SystemPromptSettings(
                ...
                currentSettings.SemanticValidationPrompt,
                useCustom
            );

            _settingsService.SaveSystemPromptSettings(newSettings);
            Log.Info("ConversationPromptActivity", "会話用プロンプトを保存");
            Toast.MakeText(this, "保存しました", ToastLength.Short)?.Show();
            Finish();
        }
        catch (Exception ex)
        {
            Log.Error("ConversationPromptActivity", $"保存エラー: {ex.Message}");
