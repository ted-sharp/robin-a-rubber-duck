...
if (-not $solutionFile) {
    Write-Host "✗ Error: No solution file (.slnx or .sln) found in current directory" -ForegroundColor Red
    exit 1
}
...
if (-not $SkipReSharper -and -not $CheckOnly) {
    ...
    if (-not $cleanupCode) {
        ...
        $settingsFile = "$solutionName.DotSettings"
